...
{if(!target)return;ctx=newCtx;setupPie();var slices=plot.getData();var attempts=0;redraw=true;while(redraw&&attempts<redrawAttempts)
{redraw=false;if(attempts>0)
maxRadius*=shrink;attempts+=1;clear();if(options.series.pie.tilt<=0.8)
drawShadow();drawPie();plot.triggerRedrawOverlay();}
...
function clear()
{ctx.clearRect(0,0,canvas.width,canvas.height);target.children().filter('.pieLabel, .pieLabelBackground').remove();total=0;processed=false;}
function drawShadow()
